...
<div class="item" style="background-image: url(<%=Model.SlideImage != null ? Model.SlideImage.Src : string.Empty %>)" >
    <div class="container">
        <div class="carousel-caption">
            <h1><%=Editable(slide => slide.Title) %></h1>
            <p class="lead"><%=Editable(slide => slide.Copy) %></p>
            <a class="btn btn-large btn-primary" href="<%=IsInEditingMode ? "#" : Model.Link.Url %>"><%=Editable(slide => slide.LinkText) %></a>
        </div>
    </div>
</div>
